...
- (void)parser:(__unused NSXMLParser *)parser
  ...
  if ([attributeDict count]) {
    ...
    case XMLDictionaryAttributesModePrefixed: {
      for (NSString *key in [attributeDict allKeys]) {
        node[[XMLDictionaryAttributePrefix stringByAppendingString:key]] =
            attributeDict[key];
      }
      break;
    }
  ...
  if (!_root) {
    ...
    if (_wrapRootNode) {
      _root =
          [NSMutableDictionary dictionaryWithObject:_root forKey:elementName];
...
+ (NSDictionary *)dictionaryWithXMLParser:(NSXMLParser *)parser {
  return
      [[[XMLDictionaryParser sharedInstance] copy] dictionaryWithParser:parser];
}
...
+ (NSDictionary *)dictionaryWithXMLString:(NSString *)string {
  return
      [[[XMLDictionaryParser sharedInstance] copy] dictionaryWithString:string];
}
...
- (NSDictionary *)attributes {
  ...
  if (attributes) {
    ...
    [filteredDict removeObjectsForKeys:@[
      XMLDictionaryCommentsKey, XMLDictionaryTextKey, XMLDictionaryNodeNameKey
    ]];
    for (NSString *key in [filteredDict allKeys]) {
      [filteredDict removeObjectForKey:key];
      if ([key hasPrefix:XMLDictionaryAttributePrefix]) {
        filteredDict[
            [key substringFromIndex:[XMLDictionaryAttributePrefix length]]] =
            self[key];
      }
    }
...
- (NSDictionary *)childNodes {
  ...
  [filteredDict removeObjectsForKeys:@[
    XMLDictionaryAttributesKey, XMLDictionaryCommentsKey, XMLDictionaryTextKey,
...
- (NSString *)innerXML {
  ...
  for (NSString *comment in [self comments]) {
    [nodes addObject:[NSString stringWithFormat:@"<!--%@-->",
...
- (NSString *)XMLString {
  if ([self count] == 1 && ![self nodeName]) {
    ...
    return [XMLDictionaryParser XMLStringForNode:self
