...
- (BOOL)registerStoreByConfig:(SCStoreConfig *)c {
  ...
  if (!store) {
    DDLogWarn(
        @"The store you tried to start:%@.%@ doesn't have a support "
...
- (BOOL)updateStoreByConfig:(SCStoreConfig *)c {
  ...
  Class store =
      [self supportedStoreByKey:[NSString stringWithFormat:@"%@.%@", c.type,
                                                           c.version]];
  SCStyle *style = c.style ? [[SCStyle alloc] initWithMapboxStyle:c.style]
                           : [[SCStyle alloc] init];
  SCDataStore *gmStore = [[store alloc] initWithStoreConfig:c withStyle:style];
  if (!store) {
    DDLogWarn(
...
- (NSDictionary *)storeAsDictionary:(SCDataStore *)ds {
  ...
  if (ds.style)
    [store setObject:[ds.style dictionary] forKey:@"style"];
