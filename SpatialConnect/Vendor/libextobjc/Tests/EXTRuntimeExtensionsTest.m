...
@property(nonatomic, strong, getter=whoopsWhatArray, setter=setThatArray:)
    NSArray *array;
...
- (void)testPropertyAttributesForBOOL {
  objc_property_t property =
      class_getProperty([RuntimeTestClass class], "normalBool");
  ...
  XCTAssertEqual(attributes->memoryManagementPolicy,
                 ext_propertyMemoryManagementPolicyAssign, @"");

  XCTAssertEqual(attributes->getter, @selector(isNormalBool), @"");
  XCTAssertEqual(attributes->setter, @selector(setNormalBool:), @"");

  XCTAssertTrue(strcmp(attributes->ivar, "_normalBool") == 0,
                @"expected property ivar name to be '_normalBool'");
  XCTAssertTrue(strlen(attributes->type) > 0,
                @"property type is missing from attributes");

  NSUInteger size = 0;
  NSGetSizeAndAlignment(attributes->type, &size, NULL);
  XCTAssertTrue(size > 0, @"invalid property type %s, has no size",
                attributes->type);
...
- (void)testPropertyAttributesForArray {
  objc_property_t property =
      class_getProperty([RuntimeTestClass class], "array");
  NSLog(@"property attributes: %s", property_getAttributes(property));
  ...
  XCTAssertTrue(strcmp(attributes->ivar, "m_array") == 0,
                @"expected property ivar name to be 'm_array'");
  XCTAssertTrue(strlen(attributes->type) > 0,
                @"property type is missing from attributes");
...
- (void)testPropertyAttributesForNormalString {
  objc_property_t property =
      class_getProperty([RuntimeTestClass class], "normalString");
  NSLog(@"property attributes: %s", property_getAttributes(property));
  ...
  XCTAssertTrue(
      strcmp(attributes->ivar, "normalString") == 0,
      @"expected property ivar name to match the name of the property");
  XCTAssertTrue(strlen(attributes->type) > 0,
                @"property type is missing from attributes");
...
- (void)testPropertyAttributesForUntypedObject {
  objc_property_t property =
      class_getProperty([RuntimeTestClass class], "untypedObject");
  NSLog(@"property attributes: %s", property_getAttributes(property));
  ...
  XCTAssertTrue(attributes->ivar == NULL,
                @"untypedObject property should not have a backing ivar");
  XCTAssertTrue(strlen(attributes->type) > 0,
                @"property type is missing from attributes");
...
- (void)testPropertyAttributesForWeakObject {
  objc_property_t property =
      class_getProperty([RuntimeTestClass class], "weakObject");
  NSLog(@"property attributes: %s", property_getAttributes(property));
  ...
  XCTAssertTrue(attributes->ivar == NULL,
                @"weakObject property should not have a backing ivar");
  XCTAssertTrue(strlen(attributes->type) > 0,
                @"property type is missing from attributes");
...
- (void)testGlobalMethodSignatureForSelector {
  XCTAssertNotNil(objc_getProtocol("EXTRuntimeTestProtocol"),
                  @"test protocol should be loaded");
  NSMethodSignature *ms =
      ext_globalMethodSignatureForSelector(@selector(optionalInstanceMethod));
  XCTAssertNotNil(ms, @"unimplemented optional protocol instance method should "
                      @"have a method signature");
  ms = ext_globalMethodSignatureForSelector(@selector(optionalClassMethod));
  XCTAssertNotNil(ms, @"unimplemented optional protocol class method should "
                      @"have a method signature");
}
