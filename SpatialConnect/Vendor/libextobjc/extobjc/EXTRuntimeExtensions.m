...
static void ext_injectSpecialProtocols(void) {
   ...
   */
  qsort_b(specialProtocols, specialProtocolCount, sizeof(EXTSpecialProtocol),
          ^(const void *a, const void *b) {
            ...
            int (^protocolInjectionPriority)(const EXTSpecialProtocol *) = ^(
                const EXTSpecialProtocol *specialProtocol) {
              int runningTotal = 0;
              ...
              for (size_t i = 0; i < specialProtocolCount; ++i) {
                ...
                if (protocol_conformsToProtocol(specialProtocol->protocol,
                                                specialProtocols[i].protocol))
                  runningTotal++;
              }
            ...
            /*
             * This will return:
             * 0 if the protocols are equal in priority (such that load order
             * does not matter) < 0 if A is more important than B > 0 if B is
             * more important than A
             */
            return protocolInjectionPriority(protoB) -
                   protocolInjectionPriority(protoA);
          });
  ...
  @autoreleasepool {
    ...
    for (size_t i = 0; i < specialProtocolCount; ++i) {
      ...
      // from the structure
      ext_specialProtocolInjectionBlock injectionBlock =
          (__bridge_transfer id)specialProtocols[i].injectionBlock;
...
BOOL ext_injectMethodsFromClass(Class srcClass, Class dstClass,
  ...
  addedCount = ext_injectMethods(dstClass, instanceMethods, count, behavior,
                                 failedToAddCallback);
  ...
  count = 0;
  Method *classMethods =
      class_copyMethodList(object_getClass(srcClass), &count);

  // ignore +load
  behavior |= ext_methodInjectionIgnoreLoad;
  addedCount = ext_injectMethods(object_getClass(dstClass), classMethods, count,
                                 behavior, failedToAddCallback);
...
Class *ext_copyClassList(unsigned *count) {
  ...
  @autoreleasepool {
    ...
    for (int i = 0; i < classCount;) {
      ...
      if (keep)
        keep &= class_respondsToSelector(class, @selector
      ...
      if (!keep) {
        if (--classCount > i) {
          memmove(allClasses + i, allClasses + i + 1,
                  (classCount - i) * sizeof(*allClasses));
        }
...
unsigned ext_addMethods(Class aClass, Method *methods, unsigned count,
  ...
  return ext_injectMethods(aClass, methods, count, behavior,
                           failedToAddCallback);
}
...
BOOL ext_addMethodsFromClass(Class srcClass, Class dstClass,
  ...
  return ext_injectMethodsFromClass(srcClass, dstClass, behavior,
                                    failedToAddCallback);
}
...
ext_propertyAttributes *ext_copyPropertyAttributes(objc_property_t property) {
  ...
  if (attrString[0] != 'T') {
    fprintf(stderr,
            "ERROR: Expected attribute string \"%s\" for property %s to start "
            "with 'T'\n",
  ...
  if (!next) {
    fprintf(stderr,
            "ERROR: Could not read past type in attribute string \"%s\" for "
            "property %s\n",
            attrString, property_getName(property));
    return NULL;
  }
  ...
  ext_propertyAttributes *attributes =
      calloc(1, sizeof(ext_propertyAttributes) + typeLength + 1);
  if (!attributes) {
    fprintf(stderr,
            "ERROR: Could not allocate ext_propertyAttributes structure for "
            "attribute string \"%s\" for property %s\n",
            attrString, property_getName(property));
    return NULL;
  }
  ...
  if (typeString[0] == *(@encode(id)) && typeString[1] == '"') {
    ...
    if (!next) {
      fprintf(stderr,
              "ERROR: Could not read class name in attribute string \"%s\" for "
              "property %s\n",
              attrString, property_getName(property));
      return NULL;
    }
  ...
  while (next && *next == ',') {
    ...
    case 'C':
      attributes->memoryManagementPolicy =
    ...
    case 'S': {
      ...
      if (!nextFlag) {
        ...
        if (!selectorLength) {
          fprintf(stderr,
                  "ERROR: Found zero length selector name in attribute string "
                  "\"%s\" for property %s\n",
                  attrString, property_getName(property));
          goto errorOut;
        }
    ...
    case 't':
      fprintf(stderr,
              "ERROR: Old-style type encoding is unsupported in attribute "
              "string \"%s\" for property %s\n",
              attrString, property_getName(property));
    ...
    default:
      fprintf(stderr,
              "ERROR: Unrecognized attribute string flag '%c' in attribute "
              "string \"%s\" for property %s\n",
  ...
  if (next && *next != '\0') {
    fprintf(stderr,
            "Warning: Unparsed data \"%s\" in attribute string \"%s\" for "
            "property %s\n",
...
Class *ext_copySubclassList(Class targetClass, unsigned *subclassCount) {
  ...
  if (!allClasses || !classCount) {
    fprintf(stderr,
            "ERROR: No classes registered with the runtime, cannot find %s!\n",
...
NSMethodSignature *ext_globalMethodSignatureForSelector(SEL aSelector) {
  ...
  static ext_methodDescription volatile methodDescriptionCache
      [selectorCacheLength];
  ...
  if (classes) {
    @autoreleasepool {
      ...
      for (uint i = 0; i < classCount; ++i) {
        ...
        if (method) {
          methodDesc = (ext_methodDescription){
              .name = aSelector, .types = method_getTypeEncoding(method)};
          break;
        }
      }
    }
  ...
  // if not found, then we can look through optional protocol methods for
  // completeness
  if (!methodDesc.name) {
    uint protocolCount = 0;
    Protocol *__unsafe_unretained *protocols =
        objc_copyProtocolList(&protocolCount);
    if (protocols) {
      struct objc_method_description objcMethodDesc;
      for (uint i = 0; i < protocolCount; ++i) {
        objcMethodDesc =
            protocol_getMethodDescription(protocols[i], aSelector, NO, YES);
        if (!objcMethodDesc.name)
          objcMethodDesc =
              protocol_getMethodDescription(protocols[i], aSelector, NO, NO);

        if (objcMethodDesc.name) {
          methodDesc = (ext_methodDescription){.name = objcMethodDesc.name,
                                               .types = objcMethodDesc.types};
          break;
        }
      }
      free(protocols);
  ...
  if (methodDesc.name) {
    ...
    // NB: there are some esoteric system type encodings that cause
    // -signatureWithObjCTypes: to fail, e.g on OS X 10.8, -[NSDecimalNumber*
    // -initWithDecimal:]. Doubt it's worth trying to catch here.
    return [NSMethodSignature signatureWithObjCTypes:methodDesc.types];
  } else {
    return nil;
  }
...
BOOL ext_loadSpecialProtocol(
    Protocol *protocol, void (^injectionBehavior)(Class destinationClass)) {
  @autoreleasepool {
    ...
    if (pthread_mutex_lock(&specialProtocolsLock) != 0) {
      fprintf(stderr,
              "ERROR: Could not synchronize on special protocol data\n");
      return NO;
    }
    ...
    if (specialProtocolCount >= specialProtocolCapacity) {
      ...
      void *restrict ptr =
          realloc(specialProtocols, sizeof(*specialProtocols) * newCapacity);
      if (!ptr) {
...
void ext_removeMethod(Class aClass, SEL methodName) {
  ...
  @autoreleasepool {
    ...
    if (superclassMethod) {
      method_setImplementation(existingMethod,
                               method_getImplementation(superclassMethod));
    } else {
...
void ext_replaceMethods(Class aClass, Method *methods, unsigned count) {
  ext_injectMethods(aClass, methods, count, ext_methodInjectionReplace, NULL);
}

void ext_replaceMethodsFromClass(Class srcClass, Class dstClass) {
  ext_injectMethodsFromClass(srcClass, dstClass, ext_methodInjectionReplace,
                             NULL);
}
...
NSString *ext_stringFromTypedBytes(const void *bytes, const char *encoding) {
  switch (*encoding) {
  case 'c':
    return @(*(char *)bytes).description;
  case 'C':
    return @(*(unsigned char *)bytes).description;
  case 'i':
    return @(*(int *)bytes).description;
  case 'I':
    return @(*(unsigned int *)bytes).description;
  case 's':
    return @(*(short *)bytes).description;
  case 'S':
    return @(*(unsigned short *)bytes).description;
  case 'l':
    return @(*(long *)bytes).description;
  case 'L':
    return @(*(unsigned long *)bytes).description;
  case 'q':
    return @(*(long long *)bytes).description;
  case 'Q':
    return @(*(unsigned long long *)bytes).description;
  case 'f':
    return @(*(float *)bytes).description;
  case 'd':
    return @(*(double *)bytes).description;
  case 'B':
    return @(*(_Bool *)bytes).description;
  case 'v':
    return @"(void)";
  case '*':
    return [NSString stringWithFormat:@"\"%s\"", bytes];
