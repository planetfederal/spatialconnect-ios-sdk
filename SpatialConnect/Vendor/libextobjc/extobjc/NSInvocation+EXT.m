...
- (BOOL)setArgumentsFromArgumentList:(va_list)args {
  ...
  for (NSUInteger i = 2; i < count; ++i) {
    ...
    case '[':
      NSLog(@"Unexpected array within method argument type code \"%s\", cannot "
            @"set invocation argument!",
            type);
      return NO;

    case 'b':
      NSLog(@"Unexpected bitfield within method argument type code \"%s\", "
            @"cannot set invocation argument!",
            type);
      return NO;

    case '{':
      NSLog(@"Cannot get variable argument for a method that takes a struct "
            @"argument!");
      return NO;

    case '(':
      NSLog(@"Cannot get variable argument for a method that takes a union "
            @"argument!");
      return NO;
    ...
    case '^':
      ...
      case 'b':
      default:
        NSLog(@"Pointer to unexpected type within method argument type code "
              @"\"%s\", cannot set method invocation!",
              type);
        return NO;
      }
    ...
    case '?': {
      ...
      NSLog(@"Assuming method argument type code \"%s\" is a function pointer",
            type);
    ...
    default:
      NSLog(@"Unexpected method argument type code \"%s\", cannot set method "
            @"invocation!",
            type);
      return NO;
    }
