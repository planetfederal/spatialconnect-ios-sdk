...
[myButton addTarget:self action:@checkselector(self, buttonAction:)
forControlEvents:UIControlEventTouchUpInside]; [otherButton addTarget:self
action:@checkselector(self, buttonAction:, withEvent:)
...
#define checkselector(TARGET, ...)                                             \
  (((void)(NO && ((void)[TARGET metamacro_foreach(                             \
                      ext_checkselector_message_iter, , __VA_ARGS__)],         \
                  NO)),                                                        \
    metamacro_foreach(ext_checkselector_selector_iter, , __VA_ARGS__)))        \
      .ext_toSelector

#define checkselector0(TARGET, SELECTOR)                                       \
  (((void)(NO && ((void)[TARGET SELECTOR], NO)), #SELECTOR)).ext_toSelector

/*** implementation details follow ***/
#define ext_checkselector_message_iter(INDEX, SELPART) SELPART(0)

#define ext_checkselector_selector_iter(INDEX, SELPART) #SELPART
