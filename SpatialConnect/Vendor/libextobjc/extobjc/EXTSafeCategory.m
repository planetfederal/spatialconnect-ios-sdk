...
static void safeCategoryMethodFailed(Class cls, Method method) {
  ...
  if (isMeta)
    fprintf(stderr,
            "ERROR: Could not add class method +%s to %s (a method by the same "
            "name already exists)\n",
            methodName, className);
  else
    fprintf(stderr,
            "ERROR: Could not add instance method -%s to %s (a method by the "
...
BOOL ext_loadSafeCategory(Class methodContainer, Class targetClass) {
  ...
  return ext_injectMethodsFromClass(methodContainer, targetClass,
                                    ext_methodInjectionFailOnAnyExisting |
                                        ext_methodInjectionIgnoreLoad,
