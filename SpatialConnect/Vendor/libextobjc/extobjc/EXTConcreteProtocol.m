...
static void ext_injectConcreteProtocol(Protocol *protocol, Class containerClass,
                                       Class class) {
  ...
  Method *cmethodList =
      class_copyMethodList(object_getClass(containerClass), &cmethodCount);
  ...
  for (unsigned methodIndex = 0; methodIndex < imethodCount; ++methodIndex) {
    ...
    if (!class_addMethod(class, selector, imp, types)) {
      fprintf(stderr,
              "ERROR: Could not implement instance method -%s from concrete "
              "protocol %s on class %s\n",
              sel_getName(selector), protocol_getName(protocol),
              class_getName(class));
    }
  }
  ...
  for (unsigned methodIndex = 0; methodIndex < cmethodCount; ++methodIndex) {
    ...
    if (!class_addMethod(metaclass, selector, imp, types)) {
      fprintf(stderr,
              "ERROR: Could not implement class method +%s from concrete "
              "protocol %s on class %s\n",
              sel_getName(selector), protocol_getName(protocol),
              class_getName(class));
    }
