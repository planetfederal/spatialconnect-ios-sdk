...
- (BOOL)open {
  ...
  sqlite3_gpkg_init(_db, NULL, NULL);
...
- (BOOL)openWithFlags:(int)flags vfs:(NSString *)vfsName {
  ...
  int err = sqlite3_open_v2([self sqlitePath], (sqlite3 **)&_db, flags,
                            [vfsName UTF8String]);
  if (err != SQLITE_OK) {
    NSLog(@"error opening!: %d", err);
    return NO;
...
- (BOOL)executeUpdate:(NSString *)sql
    ...
    for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
      ...
      if (namedIdx > 0) {
        ...
        NSString *message = [NSString
            stringWithFormat:@"Could not find index for %@", dictionaryKey];
  ...
  if (idx != queryCount) {
    NSString *message =
        [NSString stringWithFormat:@"Error: the bind count (%d) is not correct "
                                   @"for the # of variables in the query (%d) "
                                   @"(%@) (executeUpdate)",
                                   idx, queryCount, sql];
    if (_logsErrors) {
      NSLog(@"%@", message);
  ...
  if (SQLITE_DONE == rc) {
    ...
    NSString *message = [NSString
        stringWithFormat:
            @"A executeUpdate is being called with a query string '%@'", sql];
    if (_logsErrors) {
      NSLog(@"%@", message);
      NSLog(@"DB Query: %@", sql);
    }
    ...
    if (outErr) {
      *outErr = [self
          errorWithMessage:[NSString stringWithUTF8String:sqlite3_errmsg(_db)]];
    }
...
- (BOOL)startSavePointWithName:(NSString *)name error:(NSError **)outErr {
  ...
  NSString *errorMessage =
      NSLocalizedString(@"Save point functions require SQLite 3.7", nil);
  if (self.logsErrors)
    NSLog(@"%@", errorMessage);
  return NO;
#endif
...
- (void)makeFunctionNamed:(NSString *)name
         maximumArguments:(int)count
                withBlock:
                    (void (^)(void *context, int argc, void **argv))block {
