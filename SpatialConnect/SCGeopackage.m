...
- (RACSignal *)addContentsTable:(NSString *)name
                      withTypes:(NSDictionary *)types {
  return [RACSignal createSignal:^RACDisposable *(
    ...
    [self.pool inDatabase:^(FMDatabase *db) {
      if ([db tableExists:name]) {
        ...
        [existingCols enumerateObjectsUsingBlock:^(NSString *str,
  ...
  return [RACSignal createSignal:^RACDisposable *(
    ...
    [self.pool inDatabase:^(FMDatabase *db) {
      ...
      if ([db tableExists:auditTableName]) {
        ...
        while ([rs next]) {
          [existingTypes setObject:[rs stringForColumn:@"type"]
